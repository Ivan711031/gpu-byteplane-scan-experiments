...
Options parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (a == "--help" || a == "-h") {
      ...
      uint64_t v = 0;
      if (!parse_u64(need_value(a), v) || (v != 1 && v != 2)) die("invalid --plane_bytes");
      opt.plane_bytes = static_cast<int>(v);
    } else if (a == "--strategy") {
      opt.strategy = std::string(need_value(a));
    } else if (a == "--k") {
      uint64_t v = 0;
      if (!parse_u64(need_value(a), v) || v == 0 || v > 8) die("invalid --k");
      opt.single_k = true;
      opt.k_single = static_cast<int>(v);
      k_set = true;
    } else if (a == "--k_min") {
      uint64_t v = 0;
      if (!parse_u64(need_value(a), v) || v == 0 || v > 8) die("invalid --k_min");
      opt.k_min = static_cast<int>(v);
      k_min_set = true;
    } else if (a == "--k_max") {
      uint64_t v = 0;
      if (!parse_u64(need_value(a), v) || v == 0 || v > 8) die("invalid --k_max");
  ...
  if (opt.single_k && opt.k_single > planes_total) die("--k exceeds number of planes");
  if (!opt.single_k) {
    if (opt.k_min > opt.k_max) die("--k_min > --k_max");
    if (k_min_set && opt.k_min > planes_total) die("--k_min exceeds number of planes");
    if (k_max_set && opt.k_max > planes_total) die("--k_max exceeds number of planes");
  }
  // If user didn't set k bounds, clamp defaults in make_k_sweep().
  if (k_set && (k_min_set || k_max_set)) die("do not mix --k with --k_min/--k_max");
...
__global__ void scan_planes_u8_byte(const uint8_t* const* __restrict__ planes,
                                    int k,
                                    uint64_t n,
  ...
  for (uint64_t i = tid; i < n; i += stride) {
    for (int p = 0; p < k; p++) sum += static_cast<unsigned long long>(planes[p][i]);
  }
  ...
  while (base < n) {
    ...
    for (int p = 0; p < k; p++) {
      ...
      if (offset + 3 < n) {
        ...
        if (offset + 0 < n) pack |= static_cast<uint32_t>(planes[p][offset + 0]) << 0;
        if (offset + 1 < n) pack |= static_cast<uint32_t>(planes[p][offset + 1]) << 8;
        if (offset + 2 < n) pack |= static_cast<uint32_t>(planes[p][offset + 2]) << 16;
        if (offset + 3 < n) pack |= static_cast<uint32_t>(planes[p][offset + 3]) << 24;
      }
...
__global__ void scan_planes_u16(const uint16_t* const* __restrict__ planes16,
  ...
  uint64_t stride = static_cast<uint64_t>(gridDim.x) * static_cast<uint64_t>(blockDim.x);
  for (uint64_t i = tid; i < n; i += stride) {
    for (int p = 0; p < k; p++) sum += static_cast<unsigned long long>(planes16[p][i]);
  }
  block_reduce_store(sum, per_block_out);
}
...
int occupancy_grid(int device, int block_threads, int grid_mul, const void* kernel, size_t shmem) {
  ...
  if (grid < 1) grid = 1;
  if (grid > std::numeric_limits<int>::max()) grid = std::numeric_limits<int>::max();
...
RunResult run_one(const Options& opt,
  ...
  for (int i = 0; i < opt.warmup; i++) {
    if (opt.plane_bytes == 2) {
      ...
      scan_planes_u8_byte<<<grid, opt.block_threads>>>(
  ...
  if (opt.plane_bytes == 1 && opt.strategy == "shared128") overfetch = 4.0;  // 4 bytes staged/lane
...
std::vector<int> make_k_sweep(const Options& opt) {
  ...
  if (k_min < 1) k_min = 1;
  if (k_max > planes_total) k_max = planes_total;
  if (k_min > k_max) die("invalid k sweep after clamping");
...
int main(int argc, char** argv) {
  ...
  if (opt.plane_bytes == 1 && opt.strategy == "packed32") {
    ...
    for (void* p : d_planes) h_ptrs.push_back(reinterpret_cast<uint32_t*>(p));
    ...
    h_ptrs.reserve(d_planes.size());
    for (void* p : d_planes) h_ptrs.push_back(reinterpret_cast<uint8_t*>(p));
    ...
    h_ptrs.reserve(d_planes.size());
    for (void* p : d_planes) h_ptrs.push_back(reinterpret_cast<uint16_t*>(p));
  ...
  if (opt.plane_bytes == 2) {
    ...
    grid = occupancy_grid(opt.device, opt.block_threads, opt.grid_mul,
                          reinterpret_cast<const void*>(scan_planes_u8_byte), 0);
  } else if (opt.strategy == "packed32") {
    grid = occupancy_grid(opt.device, opt.block_threads, opt.grid_mul,
  ...
  for (void* p : d_planes) cuda_check(cudaFree(p), "cudaFree(plane)");
