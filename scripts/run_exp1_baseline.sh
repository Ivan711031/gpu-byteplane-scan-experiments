...
baseline_cmd=(
  ...
  --csv "$run_dir/contiguous_baseline.csv"
)
...
meta_file="$run_dir/run_meta.txt"
{
  ...
  printf 'command_contiguous_baseline=%s\n' "$(join_cmd "${baseline_cmd[@]}")"
} > "$meta_file"
...

{
  ...
  printf 'ncu --set full --target-processes all --import-source yes --source-folders %q --export %q %s\n' \
    "$ROOT_DIR" \
    "$run_dir/ncu_contiguous_baseline" "$(join_cmd "$bin" --device "$DEVICE" --n "$N" --block "$BLOCK" --grid_mul "$GRID_MUL" --warmup "$WARMUP" --iters "$ITERS" --csv "$run_dir/contiguous_baseline_ncu.csv")"
